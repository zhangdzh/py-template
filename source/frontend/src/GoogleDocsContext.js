...
export const GoogleDocsProvider = ({ children }) => {
  ...
  const fetchComments = async (accessToken) => {
    try {
      ...
      const response = await fetch(`https://leafeon-test-1b6b8e28c8d5.herokuapp.com/api/comments?access_token=${accessToken}`);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
  ...
  const fetchTasks = async (accessToken) => {
    try {
      ...
      const response = await fetch(`https://leafeon-test-1b6b8e28c8d5.herokuapp.com/api/tasks?access_token=${accessToken}`);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
